...
| "Can a stored submission be corrected afterwards?" | For an event entry, yes: `EditEntryAnswersDialog` in `orgadmin-events` reads `/application-forms/:formId/with-fields` and offers **every** field — a summary drops blanks, and the skipped question is the one being filled in — then saves `PUT /events/:id/entries/:entryId/answers`, re-validated by the same `validateSubmissionData` rules the member's own submission passed. No equivalent yet for memberships or registrations. See [CORRECTING_AN_ENTRY.md](../../docs/CORRECTING_AN_ENTRY.md) |
| "Should a form ask who the record is for?" | **No, for entries and memberships alike.** Both journeys ask it themselves — "Who is this entry for?" and "Who is this membership for?" — and write it to the record. A form that asks again holds a second name nothing reconciles. Registrations are the exception: a registration is about a horse, and there is no "who is this for" box for an animal. See [ENTRY_FORMS_DO_NOT_ASK_THE_NAME.md](../../docs/ENTRY_FORMS_DO_NOT_ASK_THE_NAME.md) |
