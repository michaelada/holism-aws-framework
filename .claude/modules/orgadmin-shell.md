...
- **Core (always available):** `dashboardModule`, `formsModule`, `settingsModule`, `paymentsModule`,
  `reportingModule`, `usersModule`, `auditModule` — all from `@itsplainsailing/orgadmin-core`.
...
- Aliases `@itsplainsailing/{components,orgadmin-core,orgadmin-events,…}` to each package's
...
| "Why doesn't a card lift when the pointer crosses it?" | Deliberate. It used to rise 4px, which reads as the page shifting under the mouse; the **shadow** still lifts, so the affordance stays. The lift was in five places — `warmTheme`'s `MuiCard`, and each of the four `DashboardCard*` components setting it again in `sx` — plus the membership type selector's rows. `theme/__tests__/warmTheme.hover.test.ts` pins it. Buttons still lift 2px, deliberately left. See [WARM_THEME_IMPLEMENTATION.md](../../docs/WARM_THEME_IMPLEMENTATION.md) |
| "Why does `size=\"small\"` do nothing to a button?" | It used to: `warmTheme`'s `MuiButton.root` sets the padding and font size for every button, so MUI's own size rules were overridden and every small button rendered as a full-size pill. `sizeSmall` now overrides it back — a global change, because the field was inert everywhere |
