...
- [x] 1. Database migration
  - [x] 1.1 Create migration file `packages/backend/src/database/migrations/010_add_payment_fee_configuration.sql`
  ...
  - [x] 1.2 Write unit test for migration SQL
...
- [x] 2. Backend service changes — Memberships
  - [x] 2.1 Update `MembershipType` interface, `CreateMembershipTypeDto`, and `UpdateMembershipTypeDto`
    - Add `fee: number` (default 0.00) and `handlingFeeIncluded: boolean` (default false)
    - _Requirements: 1.4, 2.4_

  - [x] 2.2 Update `rowToMembershipType()` mapper
    - Map `row.fee` using `parseFloat` and `row.handling_fee_included`
    - _Requirements: 1.3_

  - [x] 2.3 Update `createMembershipType()` and `updateMembershipType()` SQL and params
  ...
  - [x] 2.4 Write property test for fee round-trip persistence (Memberships)
...
- [x] 3. Backend service changes — Registrations
  - [x] 3.1 Update `RegistrationType` interface, `CreateRegistrationTypeDto`, and `UpdateRegistrationTypeDto`
    - Add `fee: number` (default 0.00) and `handlingFeeIncluded: boolean` (default false)
    - _Requirements: 3.4, 4.4_

  - [x] 3.2 Update `rowToRegistrationType()` mapper
    - Map `row.fee` using `parseFloat` and `row.handling_fee_included`
    - _Requirements: 3.3_

  - [x] 3.3 Update `createRegistrationType()` and `updateRegistrationType()` SQL and params
    - Add `fee` and `handling_fee_included` to INSERT columns/values
    - Add conditional update for both fields in UPDATE dynamic builder
    - _Requirements: 3.3, 4.4_

- [x] 4. Backend service changes — Calendar
  - [x] 4.1 Update `Calendar` interface, `CreateCalendarDto`, and `UpdateCalendarDto`
    - Add `handlingFeeIncluded: boolean` (default false)
    - _Requirements: 5.4_

  - [x] 4.2 Update `rowToCalendar()` mapper and create/update SQL
    - Map `row.handling_fee_included`
    - Add to INSERT columns/values and UPDATE dynamic builder
    - _Requirements: 5.4_

- [x] 5. Backend service changes — Merchandise
  - [x] 5.1 Update `MerchandiseType` interface, `CreateMerchandiseTypeDto`, and `UpdateMerchandiseTypeDto`
    - Add `handlingFeeIncluded: boolean` (default false)
    - _Requirements: 6.4_

  - [x] 5.2 Update `rowToMerchandiseType()` mapper and create/update SQL
    - Map `row.handling_fee_included`
    - Add to INSERT columns/values and UPDATE dynamic builder
    - _Requirements: 6.4_

- [x] 6. Frontend type changes
  - [x] 6.1 Update `packages/orgadmin-memberships/src/types/membership.types.ts`
    - Add `fee: number` and `handlingFeeIncluded: boolean` to `MembershipType` and `CreateMembershipTypeDto`
    - _Requirements: 1.4, 2.4_

  - [x] 6.2 Update `packages/orgadmin-registrations/src/types/registration.types.ts`
    - Add `fee: number` and `handlingFeeIncluded: boolean` to `RegistrationType` and `RegistrationTypeFormData`
    - _Requirements: 3.4, 4.4_

  - [x] 6.3 Update Calendar form types (inline in CalendarForm)
    - Add `handlingFeeIncluded: boolean`
    - _Requirements: 5.4_

  - [x] 6.4 Update Merchandise form types (inline in CreateMerchandiseTypePage)
    - Add `handlingFeeIncluded: boolean`
    - _Requirements: 6.4_

- [x] 7. Frontend form changes — MembershipTypeForm
  - [x] 7.1 Add fee field and handling fee toggle to MembershipTypeForm
  ...
  - [x] 7.2 Write property test for fee label currency code (Memberships)
  ...
  - [x] 7.3 Write property test for fee validation
  ...
  - [x] 7.4 Write property test for handling fee toggle visibility (Memberships)
  ...
  - [x] 7.5 Write unit tests for MembershipTypeForm payment configuration
...
- [x] 8. Frontend form changes — RegistrationTypeForm
  - [x] 8.1 Add fee field and handling fee toggle to RegistrationTypeForm
    - Same pattern as MembershipTypeForm (task 7.1)
    - _Requirements: 3.1, 3.2, 4.1, 8.1, 8.2, 8.3, 8.4_

  - [x] 8.2 Write unit tests for RegistrationTypeForm payment configuration
...
- [x] 9. Frontend form changes — CalendarForm
  - [x] 9.1 Add handling fee toggle to CalendarForm
    - Add `FormControlLabel` + `Checkbox` for handling fee included
    - Conditionally show when card-based payment method selected (Calendar has inherent pricing)
    - _Requirements: 5.1, 8.1, 8.3, 8.4_

  - [x] 9.2 Write unit tests for CalendarForm handling fee toggle
...
- [x] 10. Frontend form changes — CreateMerchandiseTypePage
  - [x] 10.1 Add handling fee toggle to CreateMerchandiseTypePage
    - Add `FormControlLabel` + `Checkbox` for handling fee included
  ...
  - [x] 10.2 Write unit tests for CreateMerchandiseTypePage handling fee toggle
...
- [x] 11. Standardise EventActivityForm (Events module)
  - [x] 11.1 Update EventActivityForm fee label to use organisation currency
  ...
  - [x] 11.2 Add `events.activities.activity.feeCurrency` translation key
  ...
  - [x] 11.3 Write property test for EventActivityForm fee label currency code
  ...
  - [x] 11.4 Write property test for EventActivityForm formatCurrency usage
  ...
  - [x] 11.5 Write unit tests for EventActivityForm standardisation
...
- [x] 12. Internationalisation
  - [x] 12.1 Add shared payment translation keys to each module's i18n
  ...
  - [x] 12.2 Write unit tests for i18n key coverage
...
- [x] 13. Final checkpoint
  - [x] 13.1 Run all unit tests and property-based tests
    - Verify no regressions in existing modules
    - Verify all new tests pass
    - _Requirements: All_

  - [x] 13.2 Verify consistent Payment_Configuration_Section across all modules
